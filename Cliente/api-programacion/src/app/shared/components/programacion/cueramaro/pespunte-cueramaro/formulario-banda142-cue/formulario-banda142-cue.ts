...
@Component({
  selector: 'app-formulario-banda142-cue',
  imports: [CommonModule],
  templateUrl: './formulario-banda142-cue.html',
  styleUrl: './formulario-banda142-cue.css',
})
export class FormularioBanda142Cue {
  ...
  crearFilaVacia(): Fila {
    ...
    return {
      ...
      cliente: '',
      semana: 0,
      departamento: 'BANDA 142 CUERÁMARO',
      linea: '',
      dia: '',
  ...
  const todasLasFilas = nuevaProgramacion.filter(p =>
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Banda 142 Cuerámaro guardada con éxito', response);
