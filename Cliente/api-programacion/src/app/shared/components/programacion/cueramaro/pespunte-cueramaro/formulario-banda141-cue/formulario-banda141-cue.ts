...
@Component({
  selector: 'app-formulario-banda141-cue',
  imports: [CommonModule],
  templateUrl: './formulario-banda141-cue.html',
  styleUrl: './formulario-banda141-cue.css',
})
export class FormularioBanda141Cue {
  ...
  crearFilaVacia(): Fila {
    ...
    return {
      ...
      cliente: '',
      semana: 0,
      departamento: 'BANDA 141 CUERÁMARO',
      linea: '',
      dia: '',
  ...
  const todasLasFilas = nuevaProgramacion.filter(p =>
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Banda 141 Cuerámaro guardada con éxito', response);
