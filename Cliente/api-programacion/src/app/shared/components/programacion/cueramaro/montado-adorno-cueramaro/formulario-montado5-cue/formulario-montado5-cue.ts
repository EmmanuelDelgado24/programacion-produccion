...
@Component({
  selector: 'app-formulario-montado5-cue',
  imports: [CommonModule],
  templateUrl: './formulario-montado5-cue.html',
  styleUrl: './formulario-montado5-cue.css',
})
export class FormularioMontado5Cue {
  ...
  crearFilaVacia(): Fila {
    ...
    return {
      ...
      cliente: '',
      semana: 0,
      departamento: 'MONTADO/ADORNO 5 CUERÁMARO',
      linea: '',
      dia: '',
...
tiempoEstandarTotal(): number{
  ...
  programacionMontado() {
    const nuevaProgramacion: Programaciones[] = this.filas().map(fila => ({
      ...
      semana: fila.semana,
      departamento: fila.departamento || 'MONTADO/ADORNO 5 CUERÁMARO',
      linea: fila.linea,
      dia: fila.dia,
      modelo: +fila.modelo(),
  ...
  const todasLasFilas = nuevaProgramacion.filter(p =>
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Montado/Adorno 5 Cuerámaro guardada con éxito', response);
