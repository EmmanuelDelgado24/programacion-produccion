...
@Component({
  selector: 'app-formulario-corte-cue',
  imports: [CommonModule],
  templateUrl: './formulario-corte-cue.html',
  styleUrl: './formulario-corte-cue.css',
})
export class FormularioCorteCue {
...
tiempoEstandarTotal(): number{
  ...
  programacionCorte() {
    const nuevaProgramacion: Programaciones[] = this.filas().map(fila => ({
      ...
      semana: fila.semana,
      departamento: fila.departamento || 'CORTE CUERÁMARO',
      linea: fila.linea,
      dia: fila.dia,
      modelo: +fila.modelo(),
  ...
  const todasLasFilas = nuevaProgramacion.filter(p =>
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Corte Cuerámaro guardada con éxito', response);
