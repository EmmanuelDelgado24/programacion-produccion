...
@Component({
  selector: 'app-formulario-banda143-cue',
  imports: [CommonModule],
  templateUrl: './formulario-banda143-cue.html',
  styleUrl: './formulario-banda143-cue.css',
})
export class FormularioBanda143Cue {
  ...
  crearFilaVacia(): Fila {
    ...
    return {
      ...
      cliente: '',
      semana: 0,
      departamento: 'BANDA 143 CUERÁMARO',
      linea: '',
      dia: '',
...
tiempoEstandarTotal(): number{
  ...
  const todasLasFilas = nuevaProgramacion.filter(p =>
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Banda 143 Cuerámaro guardada con éxito', response);
