...
@Component({
  selector: 'app-formulario-coordinado-cue',
  imports: [CommonModule],
  templateUrl: './formulario-coordinado-cue.html',
  styleUrl: './formulario-coordinado-cue.css',
})
export class FormularioCoordinadoCue {
  ...
  crearFilaVacia(): Fila {
    ...
    return {
      ...
      cliente: '',
      semana: 0,
      departamento: 'COORDINADO CUERÁMARO',
      linea: '',
      dia: '',
  ...
  const todasLasFilas = nuevaProgramacion.filter(p =>
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Coordinado Cuerámaro guardada con éxito', response);
