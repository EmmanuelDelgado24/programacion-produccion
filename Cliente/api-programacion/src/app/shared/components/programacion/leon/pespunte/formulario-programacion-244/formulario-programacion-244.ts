...
@Component({
  selector: 'app-formulario-programacion-244',
  imports: [CommonModule],
  templateUrl: './formulario-programacion-244.html',
  styleUrl: './formulario-programacion-244.css',
})
export class FormularioProgramacion244 {
  ...
  crearFilaVacia(): Fila {
    ...
    return {
      ...
      cliente: '',
      semana: 0,
      departamento: 'PESPUNTE 244 LEÓN',
      linea: '',
      dia: '',
      ...
      personal: this.storage.persistenteSignal(`personal-${uuid}`, 26),
  ...
  minutosRestantes(): number {
    ...
    const personal = this.filas()[0]?.personal() ?? 26;
  ...
  actualizarSaturacion(fila: Fila) {
    fila.saturacion.set(this.saturacionTotal());
  }
  ...
  addProgramacionPespunte244() {
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Pespunte 244 León guardada con éxito', response);
