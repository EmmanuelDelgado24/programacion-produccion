...
@Component({
  selector: 'app-formulario-programacion-245',
  imports: [CommonModule],
  templateUrl: './formulario-programacion-245.html',
  styleUrl: './formulario-programacion-245.css',
})

export class FormularioProgramacion245 {
  ...
  crearFilaVacia(): Fila {
    ...
    return {
      ...
      cliente: '',
      semana: 0,
      departamento: 'PESPUNTE 245 LEÓN',
      linea: '',
      dia: '',
      ...
      personal: this.storage.persistenteSignal(`personal-${uuid}`, 17),
  ...
  minutosRestantes(): number {
    ...
    const personal = this.filas()[0]?.personal() ?? 17;
  ...
  addProgramacionPespunte245() {
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Pespunte 245 León guardada con éxito', response);
