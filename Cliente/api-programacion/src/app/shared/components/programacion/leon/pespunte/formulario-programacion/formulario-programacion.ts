...
@Component({
  ...
  templateUrl: './formulario-programacion.html',
  styleUrl: './formulario-programacion.css',
})

export class FormularioProgramacion {
  ...
  crearFilaVacia(): Fila {
    ...
    return {
      ...
      cliente: '',
      semana: 0,
      departamento: 'PESPUNTE 242 LEÓN',
      linea: '',
      dia: '',
      ...
      personal: this.storage.persistenteSignal(`personal-${uuid}`, 25),
  ...
  recalcularFila(fila: Fila) {
    ...
    const demanda =
      fila.talla5Pares() + fila.talla55Pares() + fila.talla6Pares() + fila.talla65Pares() +
      fila.talla7Pares() + fila.talla75Pares() + fila.talla8Pares() + fila.talla85Pares() +
      fila.talla9Pares() + fila.talla95Pares() + fila.talla10Pares() + fila.talla105Pares() + fila.talla11Pares();
...
minutosRestantes(): number {
  ...
  const personal   = this.filas()[0]?.personal() ?? 25;
...
tiempoEstandarTotal(): number{
  ...
  buscarTiempoEstandar(fila: Fila) {
    const modelPes = fila.modelo().trim();
    ...
    //console.log('Programaciones enviadas:');
    //console.log(JSON.stringify(modelPes, null, 2));

    this.modelosPespunteService.getModelosPespunteEstilo(modelPes).subscribe({
      next: (res) => {
        //Si res es un array, tomas el primer elemento (res[0]), si no, asumes que res es un objeto Pespuntes
        const resultado = Array.isArray(res) ? res[0] : res as Pespuntes;
  ...
  addProgramacionPespunte242() {
    const nuevaProgramacion: Programaciones[] = this.filas().map(fila => ({
      ...
      detallesNumeracion: [
        { talla: 50,  cantidad: fila.talla5Pares()   ?? 0 },
        { talla: 55,  cantidad: fila.talla55Pares()  ?? 0 },
        { talla: 60,  cantidad: fila.talla6Pares()   ?? 0 },
        { talla: 65,  cantidad: fila.talla65Pares()  ?? 0 },
        { talla: 70,  cantidad: fila.talla7Pares()   ?? 0 },
        { talla: 75,  cantidad: fila.talla75Pares()  ?? 0 },
        { talla: 80,  cantidad: fila.talla8Pares()   ?? 0 },
        { talla: 85,  cantidad: fila.talla85Pares()  ?? 0 },
        { talla: 90,  cantidad: fila.talla9Pares()   ?? 0 },
        { talla: 95,  cantidad: fila.talla95Pares()  ?? 0 },
        { talla: 100, cantidad: fila.talla10Pares()  ?? 0 },
        { talla: 105, cantidad: fila.talla105Pares() ?? 0 },
        { talla: 110, cantidad: fila.talla11Pares()  ?? 0 },
      ].filter(d => d.cantidad > 0) // ← Solo enviamos tallas que tengan cantidad > 0
    }));
  ...
  const todasLasFilas = nuevaProgramacion.filter(p =>
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Pespunte 242 León guardada con éxito', response);
      ...
      error: (err) => {
        ...
        console.error('Errores de validación:', JSON.stringify(err.error.errors, null, 2));
