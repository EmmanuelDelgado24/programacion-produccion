...
@Component({
  selector: 'app-programacion-montado-adorno',
  imports: [CommonModule],
  templateUrl: './programacion-montado-adorno.html',
  styleUrl: './programacion-montado-adorno.css',
})
export class ProgramacionMontadoAdorno {
  ...
  crearFilaVacia(): Fila {
    ...
    return {
      ...
      cliente: '',
      semana: 0,
      departamento: 'MONTADO/ADORNO LEÓN',
      linea: '',
      dia: '',
      ...
      personal: this.storage.persistenteSignal(`personal-${uuid}`, 39),
  ...
  minutosRestantes(): number {
    ...
    const personal = this.filas()[0]?.personal() ?? 39;
  ...
  addProgramacionMontado() {
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Montado/Adorno León guardada con éxito', response);
