...
@Component({
  selector: 'app-programacion-coordinado',
  imports: [CommonModule],
  templateUrl: './programacion-coordinado.html',
  styleUrl: './programacion-coordinado.css',
})
export class ProgramacionCoordinado {
  ...
  crearFilaVacia(): Fila {
    ...
    return {
      ...
      personal: this.storage.persistenteSignal(`personal-${uuid}`, 29),
  ...
  minutosRestantes(): number {
    ...
    const personal = this.filas()[0]?.personal() ?? 29;
  ...
  /*actualizarDemanda(fila: Fila, sumaDemanda: number) {
    fila.demandaDia.set(sumaDemanda);
    this.recalcularFila(fila);
  }*/
  ...
  buscarTiempoEstandar(fila: Fila) {
    const modelCoor = fila.modelo().trim();
    ...
    this.modelosCoordinadoService.getModelosCoordinadoEstilo(modelCoor).subscribe({
      next: (res) => {
        //Si res es un array, tomas el primer elemento (res[0]), si no, asumes que res es un objeto Pespuntes
        const resultado = Array.isArray(res) ? res[0] : res as Coordinados;
  ...
  addProgramacionCoordinado() {
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Coordinado León guardada con éxito', response);
