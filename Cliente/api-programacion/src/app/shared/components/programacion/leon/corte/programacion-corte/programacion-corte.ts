...
@Component({
  selector: 'app-programacion-corte',
  imports: [CommonModule],
  templateUrl: './programacion-corte.html',
  styleUrl: './programacion-corte.css',
})
export class ProgramacionCorte {
  ...
  buscarTiempoEstandar(fila: Fila) {
    const modelCor = fila.modelo().trim();
    ...
    this.modelosCorteService.getModelosCorteEstilo(modelCor).subscribe({
      next: (res) => {
        //Si res es un array, tomas el primer elemento (res[0]), si no, asumes que res es un objeto Pespuntes
        const resultado = Array.isArray(res) ? res[0] : res as Cortes;
  ...
  addProgramacionCorte() {
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Corte León guardada con éxito', response);
