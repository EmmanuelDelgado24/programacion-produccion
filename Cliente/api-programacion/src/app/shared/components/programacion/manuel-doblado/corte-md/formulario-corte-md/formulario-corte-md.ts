...
@Component({
  selector: 'app-formulario-corte-md',
  imports: [CommonModule],
  templateUrl: './formulario-corte-md.html',
  styleUrl: './formulario-corte-md.css',
})
export class FormularioCorteMd {
...
tiempoEstandarTotal(): number{
  ...
  addProgramacionCorte() {
    const nuevaProgramacion: Programaciones[] = this.filas().map(fila => ({
      ...
      semana: fila.semana,
      departamento: fila.departamento || 'CORTE MD',
      linea: fila.linea,
      dia: fila.dia,
      modelo: +fila.modelo(),
  ...
  const todasLasFilas = nuevaProgramacion.filter(p =>
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Corte MD guardada con éxito', response);
