...
@Component({
  selector: 'app-formulario-plantilla-md',
  imports: [CommonModule],
  templateUrl: './formulario-plantilla-md.html',
  styleUrl: './formulario-plantilla-md.css',
})
export class FormularioPlantillaMd {
  ...
  tiempoEstandarTotal(): number{
    ...
    buscarTiempoEstandar(fila: Fila) {
      ...
      this.modelosPlantillaService.getModelosPlantillaEstilo(modelCor).subscribe({
        next: (res) => {
          //Si res es un array, tomas el primer elemento (res[0]), si no, asumes que res es un objeto Pespuntes
          const resultado = Array.isArray(res) ? res[0] : res as Plantillas;
  ...
  const todasLasFilas = nuevaProgramacion.filter(p =>
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Plantilla MD guardada con éxito', response);
