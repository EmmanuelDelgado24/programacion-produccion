...
@Component({
  selector: 'app-formulario-montado1-md',
  imports: [CommonModule],
  templateUrl: './formulario-montado1-md.html',
  styleUrl: './formulario-montado1-md.css',
})
export class FormularioMontado1Md {
  ...
  crearFilaVacia(): Fila {
    ...
    return {
      ...
      cliente: '',
      semana: 0,
      departamento: 'MONTADO/ADORNO 1 MD',
      linea: '',
      dia: '',
...
tiempoEstandarTotal(): number{
  ...
  const todasLasFilas = nuevaProgramacion.filter(p =>
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Montado/Adorno 1 MD guardada con éxito', response);
