...
@Component({
  selector: 'app-formulario-montado4-md',
  imports: [CommonModule],
  templateUrl: './formulario-montado4-md.html',
  styleUrl: './formulario-montado4-md.css',
})
export class FormularioMontado4Md {
  ...
  crearFilaVacia(): Fila {
    ...
    return {
      ...
      cliente: '',
      semana: 0,
      departamento: 'MONTADO/ADORNO 4 MD',
      linea: '',
      dia: '',
...
tiempoEstandarTotal(): number{
  ...
  buscarTiempoEstandar(fila: Fila) {
    ...
    this.modelosMontadoService.getModelosMontadoEstilo(modelCor).subscribe({
      next: (res) => {
  ...
  const todasLasFilas = nuevaProgramacion.filter(p =>
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Montado/Adorno 4 MD guardada con éxito', response);
