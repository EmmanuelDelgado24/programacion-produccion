...
@Component({
  selector: 'app-formulario-coordinado-md',
  imports: [CommonModule],
  templateUrl: './formulario-coordinado-md.html',
  styleUrl: './formulario-coordinado-md.css',
})
export class FormularioCoordinadoMd {
...
tiempoEstandarTotal(): number{
  ...
  addProgramacionCoordinado() {
    const nuevaProgramacion: Programaciones[] = this.filas().map(fila => ({
      ...
      semana: fila.semana,
      departamento: fila.departamento || 'COORDINADO MD',
      linea: fila.linea,
      dia: fila.dia,
      modelo: +fila.modelo(),
  ...
  const todasLasFilas = nuevaProgramacion.filter(p =>
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Coordinado MD guardada con éxito', response);
