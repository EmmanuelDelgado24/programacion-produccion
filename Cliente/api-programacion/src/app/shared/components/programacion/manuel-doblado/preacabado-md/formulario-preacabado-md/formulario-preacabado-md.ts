...
@Component({
  selector: 'app-formulario-preacabado-md',
  imports: [CommonModule],
  templateUrl: './formulario-preacabado-md.html',
  styleUrl: './formulario-preacabado-md.css',
})
export class FormularioPreacabadoMd {
...
tiempoEstandarTotal(): number{
  ...
  buscarTiempoEstandar(fila: Fila) {
    ...
    this.modelosPreacabadoService.getModelosPreacabadoEstilo(modelCor).subscribe({
      next: (res) => {
        //Si res es un array, tomas el primer elemento (res[0]), si no, asumes que res es un objeto Pespuntes
        const resultado = Array.isArray(res) ? res[0] : res as Preacabados;
  ...
  addProgramacionPreacabado() {
    ...
    const todasLasFilas = nuevaProgramacion.filter(p =>
      ...
      this.programacionService.postProgramacion(todasLasFilas).subscribe({
        next: (response) => {
          console.log('Programación de Preacabado MD guardada con éxito', response);
