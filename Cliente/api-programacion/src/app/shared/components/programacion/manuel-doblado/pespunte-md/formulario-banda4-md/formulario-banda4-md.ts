...
@Component({
  selector: 'app-formulario-banda4-md',
  imports: [CommonModule],
  templateUrl: './formulario-banda4-md.html',
  styleUrl: './formulario-banda4-md.css',
})
export class FormularioBanda4Md {
...
tiempoEstandarTotal(): number{
  ...
  const todasLasFilas = nuevaProgramacion.filter(p =>
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Banda 4 MD guardada con éxito', response);
