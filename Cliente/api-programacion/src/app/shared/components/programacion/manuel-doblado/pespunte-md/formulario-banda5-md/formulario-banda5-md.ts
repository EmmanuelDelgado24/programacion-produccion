...
@Component({
  selector: 'app-formulario-banda5-md',
  imports: [CommonModule],
  templateUrl: './formulario-banda5-md.html',
  styleUrl: './formulario-banda5-md.css',
})
export class FormularioBanda5Md {
...
tiempoEstandarTotal(): number{
  ...
  const todasLasFilas = nuevaProgramacion.filter(p =>
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Banda 5 MD guardada con éxito', response);
