...
@Component({
  selector: 'app-formulario-inyeccion-md',
  imports: [CommonModule],
  templateUrl: './formulario-inyeccion-md.html',
  styleUrl: './formulario-inyeccion-md.css',
})
export class FormularioInyeccionMd {
...
tiempoEstandarTotal(): number{
  ...
  buscarTiempoEstandar(fila: Fila) {
    ...
    this.modelosInyeccionService.getModelosInyeccionEstilo(modelCor).subscribe({
      next: (res) => {
        //Si res es un array, tomas el primer elemento (res[0]), si no, asumes que res es un objeto Pespuntes
        const resultado = Array.isArray(res) ? res[0] : res as Inyecciones;
   ...
   addProgramacionInyeccion() {
    ...
    const todasLasFilas = nuevaProgramacion.filter(p =>
      ...
      this.programacionService.postProgramacion(todasLasFilas).subscribe({
        next: (response) => {
          console.log('Programación de Inyección MD guardada con éxito', response);
