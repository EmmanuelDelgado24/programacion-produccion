...
@Component({
  selector: 'app-formulario-banda7-md',
  imports: [CommonModule],
  templateUrl: './formulario-banda7-md.html',
  styleUrl: './formulario-banda7-md.css',
})
export class FormularioBanda7Md {
...
tiempoEstandarTotal(): number{
  ...
  const todasLasFilas = nuevaProgramacion.filter(p =>
    ...
    this.programacionService.postProgramacion(todasLasFilas).subscribe({
      next: (response) => {
        console.log('Programación de Banda 7 MD guardada con éxito', response);
